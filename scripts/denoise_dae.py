"""Train the DAE denoising baseline across all k-folds (entry script).

Faithful migration of ``MAP_VAE/dae_denoising_allfolds.py`` (the root copy that
the ``submit_jobs_dae.sh`` launcher invokes). Only mechanical edits were applied:
...
``python scripts/denoise_dae.py --config=configs/config_dae.py``.
"""
import torch
...
import time
import pandas as pd
from cardiac_map_diffusion.models.dae_model import DAE
...
def main(_):
    ...
    for s_number in range(n_folds):
        print(f"\n{'='*60}")
        print(f"Starting DAE training for FOLD {s_number + 1}/{n_folds}")
        print(f"{'='*60}")
        ...
        if not cluster:
            wandb.init(
                ...
                project="DAE_pytorch_baseline",
                name=f"model_DAE_fold_{s_number}_ntype_{noise_type[:3]}_es{early_stopping_patience if early_stopping else 'off'}",
                # track hyperparameters and run metadata
                config={
                    "learning_rate": learning_rate,
                    "adam_beta1": adam_beta1,
                    "adam_beta2": adam_beta2,
                    "architecture": "DAE_conv",
        ...
        dae = DAE(z_dim=latent_size, input_dim=input_dim).float()
        dae = dae.to(device)
        ...
        if optimizer_type == "adam":
            ...
            optimizer = torch.optim.SGD(dae.parameters(), lr=learning_rate)

        # Disable scheduler for DAE baseline - use constant LR for simplicity
        ...
        for epoch in range(num_epochs):
            ...
            for i, (x, x_noisy) in enumerate(train_loader):
                ...
                # Ensure target signal has correct shape for batch size 1
                if x.ndim == 1:
                     x = x.unsqueeze(0)

                # Reshape for DAE model: expected (Batch, 1, Length, 1) or similar depending on model
                if len(x_noisy.shape) == 1: # Single Sample unbatched? Probably not, but safe check
                     x_noisy = x_noisy.unsqueeze(0).unsqueeze(0).unsqueeze(-1)
                elif len(x_noisy.shape) == 2: # (Batch, Length)
                     x_noisy = x_noisy.unsqueeze(1).unsqueeze(-1) # -> (Batch, 1, Length, 1)

                # Forward pass through DAE: input noisy, target clean
                x_recon, _ = dae(x_noisy)  # Reconstruct from noisy input
                ...
                optimizer.step()
                # Scheduler disabled for DAE baseline
                # if optimizer_scheduler:
                #     scheduler.step()
            ...
            for i_test, (x_test, x_test_noisy) in enumerate(test_loader):
                ...
                # Reshape for DAE: (Batch, 1, Length, 1)
                if len(x_test_noisy.shape) == 2:
                    x_test_noisy_reshaped = x_test_noisy.unsqueeze(1).unsqueeze(-1)
                else:
                    x_test_noisy_reshaped = x_test_noisy

                with torch.no_grad():
                    x_test_recon, _ = dae(x_test_noisy_reshaped)  # Reconstruct from noisy
                    
                    # Squeeze back for loss calculation
                    loss_test_val = torch.nn.functional.mse_loss(x_test_recon.squeeze(), x_test, reduction='mean')
            ...
            if early_stopping:
                if current_test_loss < best_test_loss:
                    ...
                    # Save best model
                    best_model_path = os.path.join(config.experiment_dir, f"DAE_best_rs_{random_seed}_fold{s_number}.pth")
                    torch.save({
        ...
        model_checkpoint_path = os.path.join(config.experiment_dir, f"DAE_rs_{random_seed}_fold{s_number}.pth")
        torch.save({
            'model_state_dict': dae.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'epoch': final_epoch,
        ...
        with torch.no_grad():
            ...
            for x_train_batch, x_train_noisy_batch in train_full:
                ...
                # Reshape for DAE: (Batch, 1, Length, 1)
                x_train_noisy_batch = x_train_noisy_batch.squeeze().float().to(device)
                if len(x_train_noisy_batch.shape) == 2:
                     x_train_noisy_input = x_train_noisy_batch.unsqueeze(1).unsqueeze(-1)
                else:
                     x_train_noisy_input = x_train_noisy_batch
                
                # Forward pass through DAE
                recon_x_train_batch, _ = dae(x_train_noisy_input)
                
                # Convert to numpy for metrics calculation (Squeeze back)
                x_train_np = x_train_batch.cpu().float().numpy()
                recon_x_train_np = recon_x_train_batch.squeeze().cpu().float().numpy()
            ...
            for x_test_batch, x_test_noisy_batch in test_full:
                ...
                # Reshape for DAE: (Batch, 1, Length, 1)
                x_test_noisy_batch = x_test_noisy_batch.squeeze().float().to(device)
                if len(x_test_noisy_batch.shape) == 2:
                     x_test_noisy_input = x_test_noisy_batch.unsqueeze(1).unsqueeze(-1)
                else:
                     x_test_noisy_input = x_test_noisy_batch
                
                # Forward pass through DAE
                recon_x_test_batch, _ = dae(x_test_noisy_input)
                
                # Convert to numpy for metrics calculation (Squeeze back)
                x_test_np = x_test_batch.cpu().float().numpy()
                recon_x_test_np = recon_x_test_batch.squeeze().cpu().float().numpy()
            ...
            if hidden_loader is not None:
                 ...
                 for x_hidden_batch, x_hidden_noisy_batch in hidden_loader:
                     ...
                     # Reshape for DAE: (Batch, 1, Length, 1)
                     x_hidden_noisy_batch = x_hidden_noisy_batch.squeeze().float().to(device)
                     if len(x_hidden_noisy_batch.shape) == 2:
                          x_hidden_noisy_input = x_hidden_noisy_batch.unsqueeze(1).unsqueeze(-1)
                     else:
                          x_hidden_noisy_input = x_hidden_noisy_batch

                     recon_x_hidden_batch, _ = dae(x_hidden_noisy_input)
                     
                     # Convert to numpy (Squeeze back)
                     x_hidden_np = x_hidden_batch.cpu().float().numpy()
                     recon_x_hidden_np = recon_x_hidden_batch.squeeze().cpu().float().numpy()
    ...
    for fold_idx in range(n_folds):
        checkpoint_name = f"DAE_rs_{random_seed}_fold{fold_idx}.pth"
    ...
    print(f"🧠 Model checkpoints: DAE_*_fold*.pth")
