"""Train the LUNet denoising baseline across all k-folds (entry script).

Faithful migration of ``MAP_VAE/lunet_denoising_allfolds.py`` (the root copy that
the ``submit_jobs_lunet.sh`` launcher invokes). Only mechanical edits were applied:
...
``python scripts/denoise_lunet.py --config=configs/config_lunet.py``.
"""
import torch
...
import time
import pandas as pd
from cardiac_map_diffusion.models.lunet_model import LUNetModel
...
def main(_):
    ...
    for s_number in range(n_folds):
        print(f"\n{'='*60}")
        print(f"Starting LUNet training for FOLD {s_number + 1}/{n_folds}")
        print(f"{'='*60}")
        ...
        if not cluster:
            wandb.init(
                ...
                project="LUNet_pytorch_baseline",
                name=f"model_LUNet_fold_{s_number}_ntype_{noise_type[:3]}_es{early_stopping_patience if early_stopping else 'off'}",
                # track hyperparameters and run metadata
                config={
                    "learning_rate": learning_rate,
                    "adam_beta1": adam_beta1,
                    "adam_beta2": adam_beta2,
                    "architecture": "LUNet_UNet",
        ...
        lunet = LUNetModel(input_dim=input_dim, adaptive=True).float()
        lunet = lunet.to(device)

        if optimizer_type == "adam":
            optimizer = torch.optim.Adam(lunet.parameters(), lr=learning_rate, 
                                       betas=(adam_beta1, adam_beta2))
        elif optimizer_type == "sgd":
            optimizer = torch.optim.SGD(lunet.parameters(), lr=learning_rate)

        # No scheduler for baseline simplicity (as fixed in DAE)
        ...
        for epoch in range(num_epochs):
            ...
            for i, (x, x_noisy) in enumerate(train_loader):
                ...
                # Forward pass through LUNet: input noisy, target clean
                x_recon = lunet(x_noisy)  # Reconstruct from noisy input
                ...
                optimizer.step()
                # Scheduler disabled for baseline simplicity
                # if optimizer_scheduler:
                #     scheduler.step()
            ...
            for i_test, (x_test, x_test_noisy) in enumerate(test_loader):
                ...
                with torch.no_grad():
                    x_test_recon = lunet(x_test_noisy)  # Reconstruct from noisy
            ...
            if early_stopping:
                if current_test_loss < best_test_loss:
                    ...
                    # Save best model
                    best_model_path = os.path.join(config.experiment_dir, f"LUNet_best_rs_{random_seed}_fold{s_number}.pth")
                    torch.save({
                        'model_state_dict': lunet.state_dict(),
                ...
                if patience_counter >= early_stopping_patience:
                    ...
                    if os.path.exists(best_model_path):
                        ...
                        lunet.load_state_dict(checkpoint['model_state_dict'])
        ...
        model_checkpoint_path = os.path.join(config.experiment_dir, f"LUNet_rs_{random_seed}_fold{s_number}.pth")
        torch.save({
            'model_state_dict': lunet.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'epoch': final_epoch,
        ...
        with torch.no_grad():
            ...
            for x_train_batch, x_train_noisy_batch in train_full:
                ...
                # Forward pass through LUNet
                recon_x_train_batch = lunet(x_train_noisy_batch.float())
            ...
            for x_test_batch, x_test_noisy_batch in test_full:
                ...
                # Forward pass through LUNet
                recon_x_test_batch = lunet(x_test_noisy_batch.float())
            ...
            if hidden_loader is not None:
                 ...
                 for x_hidden_batch, x_hidden_noisy_batch in hidden_loader:
                     ...
                     recon_x_hidden_batch = lunet(x_hidden_noisy_batch.float())
        ...
        summary_dict = {
            'architecture': 'LUNet',
            'architecture_type': 'UNet_skip_connections',
            'loss_type': 'mse',
            'n_epochs': num_epochs,
            'learning_rate': learning_rate,
            'batch_size': batch_size,
    ...
    for fold_idx in range(n_folds):
        checkpoint_name = f"LUNet_rs_{random_seed}_fold{fold_idx}.pth"
    ...
    print(f"🧠 Model checkpoints: LUNet_*_fold*.pth")
