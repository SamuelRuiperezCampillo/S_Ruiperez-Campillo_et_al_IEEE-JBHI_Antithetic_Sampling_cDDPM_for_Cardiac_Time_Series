"""DRRN baseline configuration (`ml_collections` ConfigDict).

Faithful migration of `MAP_VAE/config_drrn.py`. Consumed at runtime by the absl
`config_flags.DEFINE_config_file('config', ...)` mechanism (passed as
`--config=configs/config_drrn.py`), not imported. The only non-mechanical change
...
def get_config():
    ...
    # Model architecture
    config.model_name = 'DRRN'
    config.architecture = 'LSTM_recurrent'
    
    # Training parameters
    config.exclude_patients_file = '' # File to exclude patients from training (e.g. for hidden test set)
    config.learning_rate = 5e-5  # Same as DAE and LUNet for fair comparison
    ...
    # Adam optimizer parameters (same as DAE and LUNet)
    ...
    # DRRN-specific parameters
    config.hidden_size = 64  # LSTM hidden size as per original paper
    
    # Data parameters
    config.noise_type = 'allmixed'  # Same as DAE and LUNet for fair comparison
    config.num_folds = 4
    config.seed = 17  # Same as DAE and LUNet
    ...
    # Experiment settings
    config.experiment_name = f'DRRN_baseline_{config.noise_type}_es{config.early_stopping_patience}'
    config.working_dir = str(paths.experiments_root())  # Match DAE and LUNet path
