"""Generate the AV-vs-Crude-MC sampling LaTeX table from local (Downloads) xlsx.

Faithful migration of ``MAP_VAE/generate_local_sampling_table.py`` (a bare
top-level script run via ``__main__``). Only a module docstring was added; no
import rewrites were needed. The ``EXPERIMENTS`` dict reads one-off
``summary_values_*.xlsx`` copies from a folder configured via the
...
# Experiment Map: (Shots, Sampling Type) -> summary_values_*.xlsx file.
# The one-off summary copies live in a local folder; set it via the
# CARDIACDIFF_SAMPLING_DIR environment variable (default: ./sample_ddpm).
...
EXPERIMENTS = {
    ...
    # Note: two "av" files exist for 5 ("summary_values_5_av.xlsx" and "summary_values_5av.xlsx");
    # "summary_values_5_av.xlsx" is used for Crude and "summary_values_5av.xlsx" for AV.
...
if __name__ == "__main__":
    ...
    with open("results_table_local.tex", "w") as f:
        f.write(table_latex)
    print(f"\nSaved to {os.path.abspath('results_table_local.tex')}")
